...
inline void test_broadcast_add_row_to_matrix(Backend& be) {
    std::cout << "[Broadcast 1] add {3} + {2,3} → {2,3}: row broadcast\n";
    // b = [10, 20, 30], a = [[1,2,3],[4,5,6]] → [[11,22,33],[14,25,36]]
...
inline void test_broadcast_add_col_to_matrix(Backend& be) {
    std::cout << "[Broadcast 2] add {2,1} + {2,3} → {2,3}: column broadcast\n";
    // a = [[1],[2]], b = [[10,20,30],[40,50,60]] → [[11,21,31],[42,52,62]]
...
inline void test_broadcast_add_scalar_to_matrix(Backend& be) {
...
inline void test_broadcast_add_incompatible_throws(Backend& be) {
...
inline void test_broadcast_backward_row_to_matrix(Backend& be) {
    std::cout << "[Broadcast 5] backward {3}+{2,3}: row grad reduces over batch dim\n";
    // a (requires_grad) = [1,2,3], b = [[4,5,6],[7,8,9]]
    // d loss/d a_j = 2 (appears in 2 rows) → grad_a = [2,2,2]
...
inline void test_broadcast_backward_col_to_matrix(Backend& be) {
    std::cout << "[Broadcast 6] backward {2,1}+{2,3}: col grad reduces over cols\n";
    // a (requires_grad) = [[1],[2]], b = [[10,20,30],[40,50,60]]
    // d loss/d a_i0 = 3 (appears in 3 columns) → grad_a = [[3],[3]]
    Tensor a = Tensor::from_data<double>({1, 2},                {2, 1}, be, /*requires_grad=*/true);
    Tensor b = Tensor::from_data<double>({10, 20, 30, 40, 50, 60}, {2, 3}, be);
    a.add(b).sum().backward();
    CHECK_NEAR(a.grad().at({0, 0}), 3.0, 1e-12);
    CHECK_NEAR(a.grad().at({1, 0}), 3.0, 1e-12);
}

inline void test_broadcast_backward_both_inputs_need_reduction(Backend& be) {
    ...
    a.add(b).sum().backward();
    // grad_a: each element appears in 2 rows → [2,2,2]
    CHECK_NEAR(a.grad().at({0}), 2.0, 1e-12);
    CHECK_NEAR(a.grad().at({1}), 2.0, 1e-12);
    CHECK_NEAR(a.grad().at({2}), 2.0, 1e-12);
    // grad_b: each element appears once → all 1s
...
inline void run_shared_broadcast(Backend& be) {
    test_broadcast_add_row_to_matrix(be);
    test_broadcast_add_col_to_matrix(be);
    test_broadcast_add_scalar_to_matrix(be);
    test_broadcast_add_incompatible_throws(be);
    test_broadcast_backward_row_to_matrix(be);
    test_broadcast_backward_col_to_matrix(be);
    test_broadcast_backward_both_inputs_need_reduction(be);
}
